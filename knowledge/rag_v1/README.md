...
| 条件 | 行为 |
|---|---|
| 命令行 `--force` | 清空整个集合，全量重建 |
| 文档 YAML `force: true` | 删除该文档旧块，重新嵌入 |
| 文档 `crawledAt` 晚于库中已存储的 `crawledAt` | 自动删除旧块，重新嵌入 |
| 文档 `crawledAt` 未变化，且 `force: false` | 跳过 |
| 文档无 `crawledAt` 字段，且 `force: false` | 跳过（与原逻辑相同） |
